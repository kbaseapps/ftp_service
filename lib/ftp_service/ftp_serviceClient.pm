package ftp_service::ftp_serviceClient;
...
sub new
{
    ...
    my $self = {
	client => ftp_service::ftp_serviceClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
# Authentication: required
    ...
    {
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function list_files (received $n, expecting 1)");
    }
    {
	my($params) = @args;
...
sub version {
    ...
    if ($result) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(
            error => "Error invoking method list_files",
            status_line => $self->{client}->status_line,
            method_name => 'list_files',
        );
    }
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for ftp_service::ftp_serviceClient\n";
    }
    if ($sMajor == 0) {
        warn "ftp_service::ftp_serviceClient version is $svr_version. API subject to change.\n";
    }
}
...
package ftp_service::ftp_serviceClient::RpcClient;
